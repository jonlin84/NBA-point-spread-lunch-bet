...
class SpreadModel(object):
    ...
    def __init__(self):
        ...
        self.avg_5_no_pct_diff = [
                        'team_mp_last_avg_diff',
    ...
    def _current_get_rolling_avg(self, team, year, rolling_avg):
        ...
        ats_record = np.insert((
                        np.cumsum(sp.ats.values[:len(sp)-1]))
        ...
        team_df = box_df[
                (box_df['team'] == team) & (box_df['year'] == year)].copy()
    ...
    def _transform_spread(
            self, year, rolling_avg, without_first_games, spread_home_team):
        '''
        ...
        for i in range(len(spread)):
            ...
            team = spread['team'][i]
            opp = spread['opp'][i]
            team_stat = rolling_avg_season[
                                (rolling_avg_season['game_id'] == gameid)
                                & (rolling_avg_season['team'] == team)].copy()
            opp_stat = rolling_avg_season[
                                (rolling_avg_season['game_id'] == gameid)
                                & (rolling_avg_season['team'] == opp)].copy()
    ...
    def matchup_predict_data(self, home: str, away: str, spread: float):
        ...
        '''
        home_rolling = self._current_get_rolling_avg(
                                        home, self.current_season, 6)
        away_rolling = self._current_get_rolling_avg(
                                    away, self.current_season, 6)
        ...
        diff = pd.concat(
                        [pd.DataFrame(h_off).T, pd.DataFrame(o_off).T], axis=1)
        diff.columns = self.avg_5_no_pct_diff
        ...
        data = {
                'g': [g], 'spread': [spread], 'team_last_5': [team_5],
