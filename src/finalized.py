...
def transform_spread(spread,teams,year,box,team_avg,rolling_avg=5):
    ...
    spread = spread[spread['year']==year].copy()
    ...
    for i in range(len(spread)):
        ...
        team_ats = rolling_avg_season['ats_record'].loc[(rolling_avg_season['game_id']==gameid)&(rolling_avg_season['team']==team)].values
        opp_ats = rolling_avg_season['ats_record'].loc[(rolling_avg_season['game_id']==gameid)&(rolling_avg_season['team']==opp)].values
        #print(team_ats)
        diff = pd.DataFrame(team_stat[avg_5_no_pct].values - opp_stats)
        diff.columns = avg_5_no_pct_diff
        diff['team_ats'] = team_ats
        diff['opp_ats'] = opp_ats
